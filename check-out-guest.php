...
{
  //redirect
  header('Location: check-out.php?cannot-find-guest');
}
else
{
  $guest->free_result();

  $res = $db->conn->prepare("UPDATE RESERVE SET check_out=1 WHERE guest_id=$guestID AND room_id=?");
  ...
  $resExecute = $res->execute();
    ...
    if(!$res)
    {
      header('Location: check-out.php?cannot-check-out');
    }
    else
    {
