...
$stmt = $db->conn->prepare("SELECT guest_id FROM GUEST WHERE first_name=? AND last_name=?");
...
if(!$guestID)
{
  header('Location: show-data.php?could-not-find-guest');
}

$stmt->free_result();
////////////////////////////////////////////////

//$statement = $db->query("SELECT * FROM RESERVE WHERE guest_id = $guestID");
$statement = $db->query("SELECT * FROM RESERVE AS R, GUEST AS G WHERE G.GUEST_ID=R.GUEST_ID AND G.guest_id = $guestID");
$i = 0;
$reservations = array();
...
while($row = $statement->fetch_assoc())
{
  ...
  echo 'Check In Date: ' .$row['RES_START']. "<br>";
  echo 'Check Out Date: ' .$row['RES_END']. "<br>";
...
//header('Location: show-data.php?guest='.$guestID);

//echo "<p> Guest's First Name: " $guestFirst "</p>";
//echo "<p> Guest's First Name: " $guestLast "</p>";

//foreach($reservations as $reservation){
  //echo 'Room Number: ' .$reservation['room_id']. "<br>";
  //echo "<p> Check In Date: " $reservation['res_start'] "</p>";
  //echo "<p> Check Out Date: " $reservation['res_end'] "</p>";}
